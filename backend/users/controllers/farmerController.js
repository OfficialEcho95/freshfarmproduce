...
const addCommodity = async (req, res) => {
  try {
    ...
    const {
      title, description, price, quantityAvailable, categories, location,
    } = req.body;
...
const salesReport = async (req, res) => {
  try {
    ...
    const data = orders.map((order) => order.items.map((item) => ({
...
const productsTracker = async (req, res) => {
  try {
    ...
    const publishPromises = lowCommodities.map((comm) => redisClient.publish('running low on product', JSON.stringify({
      email: comm.email,
